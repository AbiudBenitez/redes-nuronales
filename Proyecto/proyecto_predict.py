...
model = load_model(
    "modelo_mejorado_glove.h5", custom_objects={"focal_loss_fixed": focal_loss()}
)
...
opcion = (
    input("¿Deseas conservar las filas sin guion (encabezados)? [s/n]: ")
    .strip()
    .lower()
)
...
df_desglosadas["CUENTA"] = (
    df_desglosadas["CUENTA"].astype(str).str.strip().apply(limpiar_texto)
)
df_desglosadas["InputText"] = (
    "CUENTA " + df_desglosadas["No CUENTA"] + " NOMBRE " + df_desglosadas["CUENTA"]
)
...
if conservar_encabezados:
    df_resultado = pd.concat(
        [df_desglosadas, df_encabezados], ignore_index=True
    ).sort_index()
else:
